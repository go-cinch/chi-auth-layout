...
{{ end }}
	if err != nil { server.WriteError(w, r, http.StatusBadRequest, apperror.WhitelistID); return }
...
{{ end }}
	...
	if values, provided := query["category"]; provided {
		if len(values) != 1 { server.WriteError(w, r, http.StatusBadRequest, apperror.CategoryType); return }
		parsed, err := strconv.ParseInt(values[0], 10, 16)
		if err != nil || parsed < 0 || parsed > 1 { server.WriteError(w, r, http.StatusBadRequest, apperror.Category); return }
		value := int16(parsed); input.Category = &value
	}
